...
class VarNetDataTransform:
    ...
    def __call__(
        ...
        """
        kspace = to_tensor(kspace)
        target = ifft2c(kspace)[:,100:-100,...]
        kspace = fft2c(target)

        max_value = rss_complex(target).max()
        seed = None if not self.use_seed else tuple(map(ord, fname))
...
def get_random_patch(img, patch_size):
    ...
    hi = randint(0, img.shape[1] - patch_size + 1) 
    wi = randint(0, img.shape[2] - patch_size + 1)
    img = crop_image(img, (hi, hi + patch_size), (wi, wi + patch_size))
    return img

class DenoisingDataTransform:
    """
    Data Transformer for training VarNet models.
    """
    ...
    def __call__(
        ...
        image_target = get_random_patch(target, self.patch_size)
        image_target = image_target / max_value
        if np.random.rand(1)<0.5:
            image_target = image_target.flip(dims=[1])
        if np.random.rand(1)<0.5:
            image_target = image_target.flip(dims=[2])
        if np.random.rand(1)<0.5:
            image_target = image_target.permute(0,2,1,3)
