...
from .tvcs.ROF import ROF_torch

class DAPnP(torch.nn.Module):
    def __init__(self, 
        scorenet=None, 
        cnn_denoisers=None,
        gamma=1.06,
    ...
    def forward(self, data_kspace, mask, sense_map=None, target_rss=None, normlize_input=False, K_gd_iter=5, max_iter=100,verbose=False):
        ...
        for idx in range(max_iter):
            ...
            with torch.no_grad():
                ...
                #-----------------------------------------------
                if self.reg_cnn:
                    v = inputs.permute(0,3,1,2)
                    v = self.cnn_denoisers(v, math.sqrt(self.lam_D / rho_k))
                    v = v.permute(0,2,3,1)
            ...
            if verbose and (target_rss is not None) and (idx%10 == 0):
                print(f'iter: {idx}, sigma: {int(sigma*255), int(math.sqrt(self.lam_D / rho_k)*255)}, PSNR: {PSNR}, SSIM: {SSIM}, TIME: {time.time()-time1}')
...
#     def data_consistancy_with_K(self, K_gd_iter, x, v_sub_u, data_kspace, mask, rho_k, sense_map):
#         self.K.eval()
#         x_tilde = FT(v_sub_u, sense_map)
#         grad_norm = 1e8
#         for _ in range(K_gd_iter):
#             x = FT(x, sense_map)
#             grad = self.K(x) - x
#             grad_norm_new = grad.pow(2).sum(0).sum(-1).mean().sqrt()
#             if grad_norm_new < 1.02*grad_norm:
#                 grad_ss = self.K.get_GTx(grad) - grad
#                 # grad_ss = grad_ss / grad_ss.pow(2).sum(0).sum(-1).sqrt().max()
#                 x = (rho_k * (x_tilde + x - self.K_step * grad_ss) + data_kspace)/(2*rho_k + mask)
#             else:
#                 x = (rho_k * (x_tilde + x) + data_kspace)/(2*rho_k + mask)
#             x = iFT(x, sense_map, COIL_DIM)
